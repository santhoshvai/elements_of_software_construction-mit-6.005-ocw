...
public class AlphabetGenerator {
    ...
    public static char[] generateFrequencyAlphabet(int base,
    	...
    	// Count the occurrence of each character a-z in trainingData
    	for (String st: trainingData ) {
    		st.replaceAll("\\s", ""); // replace all white spaces
    		for (int i = 0; i < st.length(); i++){ // iterate over each char in the string
    		    char c = st.charAt(i);
    	...
    	// find CDF*base and form the output char array
    	double prev_value = 0;
    	char[] output = new char[base];
    	int prev_cdf_base = 0;
    	for (char key : map.keySet()) {
    	    int occurence = map.get(key);
    	    double value = occurence/(double)total_num_chars; // PDF
    	    prev_value += value; // CDF
    	    int cdf_base = (int)Math.floor(prev_value*base);
    	    for (int i = prev_cdf_base; i < cdf_base; i++) {
    			output[i] = key;
    		}
    		prev_cdf_base = cdf_base;
    	}
        return output;
    }
